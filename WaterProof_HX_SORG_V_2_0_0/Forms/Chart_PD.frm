...
Begin VB.Form Chart_PD 
   Caption         =   "WaterProof HX - Heat Exchangers Performance Monitoring - Tube-side pressure drop  trend chart"
   ClientHeight    =   9720
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   14700
   ...
   Begin MSComctlLib.TabStrip TabStrip1 
      ...
      EndProperty
   End
   Begin MSChart20Lib.MSChart MSChart_PD 
      Height          =   6555
      Left            =   120
      OleObjectBlob   =   "Chart_PD.frx":0000
      TabIndex        =   0
      Top             =   1200
      Width           =   11415
   End
...
Private Sub Spin_MAX_Change()
    MM2 = Spin_MAX.Value / 1
    Text_MAX.Text = Spin_MAX / 1
    Call chart
End Sub
Private Sub Spin_MIN_Change()
    Min = Spin_MIN.Value / 1
    Text_MIN.Text = Spin_MIN / 1
    Call chart
End Sub
            ...
            CONTO = Rs1.RecordCount
                ...
                ETX(www) = Data1.Recordset.date_test
                    If ETX(www) >= INIZIO And ETX(www) <= FINE Then
                        XXX = XXX + 1
                        SY(XXX, 1) = Data1.Recordset.TUBES_PRESS_DROP
