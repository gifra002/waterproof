...
Begin VB.Form Chart_CFAC 
   Caption         =   "WaterProof HX - Heat Exchangers Performance Monitoring - C-Factor trend chart "
   ClientHeight    =   10005
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   14700
   ...
   Begin MSComctlLib.TabStrip TabStrip1 
      ...
      EndProperty
   End
   Begin MSChart20Lib.MSChart MSChart_CFAC 
      Height          =   6555
      Left            =   120
      OleObjectBlob   =   "Chart_CFAC.frx":0000
      TabIndex        =   0
      Top             =   1200
      Width           =   11295
   End
...
Private Sub Spin_MAX_Change()
    MM2 = Spin_MAX.Value * 10
    Text_MAX.Text = Spin_MAX * 10
    Call chart
End Sub
Private Sub Spin_MIN_Change()
    Min = Spin_MIN.Value * 10
    Text_MIN.Text = Spin_MIN * 10
    Call chart
End Sub
            ...
            CONTO = Rs1.RecordCount
                ...
                ETX(www) = Data1.Recordset.date_test
                    If ETX(www) >= INIZIO And ETX(www) <= FINE Then
                        XXX = XXX + 1
                        SY(XXX, 1) = Data1.Recordset.C_Factor
        ...
        .RowCount = 1
             With MSChart_CFAC.Plot
              ...
              If Check_MAX = Unchecked Then
                    MM2 = Int(MM2 * 20) / 10
                    Min = Int(NN2 / 20) * 10
                    Spin_MAX.Value = MM2 / 10
                    Spin_MIN.Value = Min / 10
                Else
                    MM2 = Spin_MAX * 10
                    Min = Spin_MIN * 10
              End If
                ...
                If FLUID = "Steam" Then
                      .Axis(VtChAxisIdY).AxisTitle = "Cleanliness (%)"
                      .Axis(VtChAxisIdY2).AxisTitle = "Cleanliness (%)"
                Else
                      .Axis(VtChAxisIdY).AxisTitle = "C Factor"
                      .Axis(VtChAxisIdY2).AxisTitle = "C Factor"
                End If
            End With
           
        If FLUID = "Steam" Then
          LE(1) = "Cleanliness,%"
          .Footnote.Text = "Trend Chart: Cleanliness,%"
        Else
          LE(1) = "CFAC"
          .Footnote.Text = "Trend chart: C Factor (m3/hr.kPa)"
        End If
